# -*- coding: utf-8 -*-
"""
Created on Sun Oct 25 18:03:25 2020
...
from bins import step_bin, rate_bin_transfer
...
iv_var_ls = ins_clean_class_df[~ins_clean_class_df['Dclass'].isin(['Key','Target'])]['index'].tolist()
...
## 生成最终入模变量数据集
keep_var_list = [x for x in var_high_predict_list if x not in manual_drop_list]
ins_ived_class_df = ins_clean_class_df[ins_clean_class_df['Dclass'].isin(['Key','Target']) 
                                     | ins_clean_class_df['index'].isin(keep_var_list)]
...
bin_rst = step_bin(inDf=ins_clean_df, inTypeDf=ins_ived_class_df, varKey=var_key, varTarget=var_target)
...
for var_name in ins_clean_class_df[ins_clean_class_df['Dtypes'] != 'object']['index'].tolist():
    ...
    if ins_clean_class_df[ins_clean_class_df['index'] == var_name]['NmissRate'].tolist()[0] <= 0.05:
        median_value = ins_clean_df[var_name].median()
        model_raw_df[var_name] = model_raw_df[var_name].fillna(0)
